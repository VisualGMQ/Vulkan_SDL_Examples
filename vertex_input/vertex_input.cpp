...
class App {
 ...
 private:
    ...
    void createVertexBuffer() {
        ...
        create_info.size = sizeof(Vertex)*TriangleVertices.size();
        create_info.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
        create_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

        assertm("create vertex buffer failed", vkCreateBuffer(device_, &create_info, nullptr, &vertex_buffer_) == VK_SUCCESS);
        ...
        allocate_info.memoryTypeIndex = findMemoryType(requirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT|VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);

        assertm("can't allocate memory", vkAllocateMemory(device_, &allocate_info, nullptr, &vertex_buf_memory_) == VK_SUCCESS);
        
        vkBindBufferMemory(device_, vertex_buffer_, vertex_buf_memory_, 0);

        void* data;
        vkMapMemory(device_, vertex_buf_memory_, 0, create_info.size, 0, &data);
        memcpy(data, TriangleVertices.data(), create_info.size);
        vkUnmapMemory(device_, vertex_buf_memory_);
    }
